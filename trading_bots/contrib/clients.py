...
class BaseClient(ClientWrapper, abc.ABC):
    ...
    def _currencies(self) -> Set[str]:
        ...
        return {
            *{market.base for market in markets},
    ...
    def exception(
        self, exc_type: Type[Exception] = None, msg: str = None, exc: Exception = None
    ) -> Exception:
        if exc_type is None:
    ...
    def check_credentials(self):
        if not self.credentials:
            raise self.exception(
                AuthenticationError, f"{self.name} credentials not set!"
            )
        for key, value in self.credentials.items():
            if not value:
                raise self.exception(
                    AuthenticationError, f"{self.name} requires `{key}`"
                )
...
class MarketClient(BaseClient, abc.ABC):
    ...
    def _parse_trades(self, trades: List, since: int) -> List[Trade]:
        return self._sort_timestamp(
            self._filter_since([self._parse_trade(trade) for trade in trades], since)
        )
...
class WalletClient(BaseClient, abc.ABC):
    ...
    # Transactions
    def _transactions(
        self, func: Callable, tx_name: str, limit: int = None
    ) -> List[Transaction]:
        return self._fetch_limit(tx_name, self.currency)(func)(limit)

    def _transactions_since(
        self, func: Callable, tx_name: str, since: int
    ) -> List[Transaction]:
    ...
    def request_withdrawal(
        ...
        if self.dry_run:
            withdrawal = Withdrawal.create_default(
                TxType.WITHDRAWAL, self.currency, amount, address
            )
            self.log.warning(
    ...
    def _parse_transactions(self, txs: List, tx_type: TxType):
        def wrapper(filter_func: Callable, *filter_args) -> List[Transaction]:
            ...
            return self._sort_timestamp(
                filter_func(self._filter_currency(parsed_txs), *filter_args)
            )

        return wrapper

    def _parse_transactions_limit(
        self, txs: List, tx_type: TxType, limit: int = None
    ) -> List[Transaction]:
        return self._parse_transactions(txs, tx_type)(self._filter_limit, limit)

    def _parse_transactions_since(
        self, txs: List, tx_type: TxType, since: int
    ) -> List[Transaction]:
...
class TradingClient(MarketClient, abc.ABC):
    ...
    class Wallets:
        def __init__(
            ...
            **kwargs,
            ...
            self.base: WalletClient = cls(
                market.base, client_params, dry_run, logger, store, name, **kwargs
            )
            self.quote: WalletClient = cls(
                market.quote, client_params, dry_run, logger, store, name, **kwargs
            )
    ...
    def fetch_order(self, order_id: str) -> Order:
        ...
        return self._fetch(f"order id={order_id}", exc=OrderNotFound)(self._order)(
            order_id
        )
    ...
    def cancel_order(self, order_id: str) -> str:
        ...
        try:  # Cancel order
            ...
            raise self.exception(
                OrderNotFound, f"Failed to cancel order: id={order_id}", e
            ) from e
    ...
    def min_order_amount(self) -> Money:
        ...
        return self._fetch("minimum order amount", self.market.code)(
            self._min_order_amount
        )()
    ...
    def place_order(
        self, side: Side, order_type: OrderType, amount: Number, price: Number = None
    ) -> Order:
        """Place an order."""
        ...
        )  # Don´t place order if the amount < min allowed
    ...
    def _parse_orders(self, orders: List):
        def wrapper(filter_func: Callable, *filter_args) -> List[Order]:
            ...
            return self._sort_timestamp(
                filter_func(self._filter_market(parsed_orders), *filter_args)
            )
