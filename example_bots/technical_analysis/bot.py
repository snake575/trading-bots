...
from trading_bots.utils import get_iso_time_str, truncate_money, truncate_to
...
class TechnicalAnalysis(Bot):
    ...
    def get_amount(self, side):
        if side == Side.BUY:
            ...
            amount_buy = truncate_money(
                self.buda.fetch_order_book().quote(side, amount_quote).base_amount
            )
            return amount_buy
        elif side == Side.SELL:
    ...
    def get_trades(self, from_timestamp: float):
        ...
        if prev_trades:
            ...
            self.log.debug(
                f"{len(prev_trades)} previous trades: (%s ... %s)",
                get_iso_time_str(first_trade),
        ...
        trades = [
            trade for trade in prev_trades if trade.get("timestamp") >= from_timestamp
        ]
        ...
        while trades_n == 1000:
            self.log.debug(
                f"Fetching {self.reference.name} trades since: %s",
            ...
            if last_trades:
                self.log.debug(
                    f"Trades found: {len(last_trades)} (%s ... %s)",
    ...
    def _get_trades_call(self, query_timestamp: float):
        ...
        time_interval = (
            "day" if time.time() - query_timestamp >= one_day_seconds else "hour"
        )
        trades = self.reference.client.transactions(
            self.reference.market_id, time_interval
        )
        trades = [
            dict(
                timestamp=float(trade["date"]),
                rate=float(trade["price"]),
                amount=float(trade["amount"]) * (1 if trade["type"] == "0" else -1),
            )  # '0': buy, '1': sell
            for trade in trades
